...
namespace {
    ...
    [[nodiscard]] std::string pretty_error(std::size_t cursor, std::string_view chars) {
        ...
        // don't want to include <algorithm> just for a single std::count()
        ...
        for (; line_start > 0; --line_start) {
            if (chars[line_start - 1] == '\n' || cursor - line_start >= max_left_width)
                break;
        }
        ...
        // fits into SSO buffer in almost all cases
        const std::string line_prefix = std::format("Line {}: ", line_number);
...
namespace utl::internal {
    ...
    Node::object_type& Node::as_object() { return as<object_type>(); }
    Node::array_type& Node::as_array() { return as<array_type>(); }
    Node::string_type& Node::as_string() { return as<string_type>(); }
    Node::integral_type& Node::as_integral() { return as<integral_type>(); }
    Node::floating_type& Node::as_floating() { return as<floating_type>(); }
    Node::bool_type& Node::as_bool() { return as<bool_type>(); }
    Node::null_type& Node::as_null() { return as<null_type>(); }

    const Node::object_type& Node::as_object() const { return as<object_type>(); }
    const Node::array_type& Node::as_array() const { return as<array_type>(); }
    const Node::string_type& Node::as_string() const { return as<string_type>(); }
    const Node::integral_type& Node::as_integral() const { return as<integral_type>(); }
    const Node::floating_type& Node::as_floating() const { return as<floating_type>(); }
    const Node::bool_type& Node::as_bool() const { return as<bool_type>(); }
    const Node::null_type& Node::as_null() const { return as<null_type>(); }

    bool Node::is_object() const noexcept { return is<object_type>(); }
    bool Node::is_array() const noexcept { return is<array_type>(); }
    bool Node::is_string() const noexcept { return is<string_type>(); }
    bool Node::is_integral() const noexcept { return is<integral_type>(); }
    bool Node::is_floating() const noexcept { return is<floating_type>(); }
    bool Node::is_bool() const noexcept { return is<bool_type>(); }
    bool Node::is_null() const noexcept { return is<null_type>(); }
    ...
    const Node& Node::operator[](std::string_view key) const {
        ...
        if (it == object.end())
            throw ers::make_out_of_range_error("Accessing non-existent key '{}' in JSON object.", key);
        return it->second;
    }
    ...
    Node& Node::operator[](std::size_t pos) { return as_array()[pos]; }
    const Node& Node::operator[](std::size_t pos) const { return as_array()[pos]; }

    Node& Node::at(std::size_t pos) { return as_array().at(pos); }
    const Node& Node::at(std::size_t pos) const { return as_array().at(pos); }

    void Node::push_back(const Node& node) {
        if (is_null())
            _data = array_type {};
        as_array().emplace_back(node);
    }

    void Node::push_back(Node&& node) {
        if (is_null())
            _data = array_type {};
        as_array().emplace_back(std::move(node));
    }
    ...
    Node::Node(const object_type& value) { _data = value; }
    Node::Node(object_type&& value) { _data = std::move(value); }
    Node::Node(const array_type& value) { _data = value; }
    Node::Node(array_type&& value) { _data = std::move(value); }
    Node::Node(std::string_view value) { _data = string_type(value); }
    Node::Node(const string_type& value) { _data = value; }
    Node::Node(string_type&& value) { _data = std::move(value); }
    Node::Node(integral_type value) { _data = value; }
    Node::Node(floating_type value) { _data = value; }
    Node::Node(bool_type value) { _data = value; }
    Node::Node(null_type value) { _data = value; }
...
namespace {
    ...
    //    else if (c == '\t') { chars += 't' }
    // we get:
    //    if (const char replacement = lookup_serialized_escaped_chars[to_u8(c)]) { chars += replacement; }
    //
...
namespace utl::internal {
    ...
    std::size_t parser::skip_nonsignificant_whitespace(std::size_t cursor) const {
        while (cursor < chars.size()) {
            if (!lookup_whitespace_chars[to_u8(chars[cursor])]) return cursor;
            ++cursor;
        }

        throw ers::make_parse_error("JSON parser reached the end of buffer at pos {} while skipping insignificant whitespace segment. {}",
            cursor, pretty_error(cursor, chars));
    }

    std::pair<std::size_t, Node> parser::parse_node(std::size_t cursor) {
        ...
        if (('0' <= c && c <= '9') || (c == '-')) {
            switch (auto [ptr, number] = parse_number(cursor); number.index()) {
                case 0:
        ...
        throw ers::make_parse_error("Json node selector encountered expected marker symbol '{}' at pos {} (should be one of '0123456789{{[\"tfn'). {}",
    ...
    std::size_t parser::parse_object_pair(std::size_t cursor, Object& parent) {
        ...
        if (++recursion_depth > recursion_limit) {
            throw ers::make_parse_error("JSON parser has exceeded maximum allowed recursion depth of {}. "
                "If stated depth wasn't caused by an invalid input, recursion limit can be increased with json::set_recursion_limit().",
                recursion_limit);
        }

        Node value;
        std::tie(cursor, value) = parse_node(cursor);
    ...
    std::pair<std::size_t, Object> parser::parse_object(std::size_t cursor) {
        ...
        cursor = skip_nonsignificant_whitespace(cursor);
        if (chars[cursor] != '}') {
            cursor = parse_object_pair(cursor, object_value);
        } else {
        ...
        while (cursor < chars.size()) {
            ...
            if (c == ',') {
                ...
                throw ers::make_parse_error("JSON array node could not find comma ',' or object ending symbol '}}' after the element at pos {}. {}",
                    cursor, pretty_error(cursor, chars));
            }
        }

        throw ers::make_parse_error("JSON object node reached the end of buffer while parsing object contents. {}",
    ...
    std::pair<std::size_t, Array> parser::parse_array(std::size_t cursor) {
        ...
        cursor = skip_nonsignificant_whitespace(cursor);
        if (chars[cursor] != ']') {
            cursor = parse_array_element(cursor, array_value);
        } else {
            ++cursor; // move past the closing bracket ']'
            return { cursor, std::move(array_value) };
        }

        // Handle other pairs (the exact same way we do with objects, see the note here)
        ...
        while (cursor < chars.size()) {
            ...
            if (c == ',') {
                ...
                throw ers::make_parse_error("JSON array node could not find comma ',' or array ending symbol ']' after the element at pos {}. {}",
                    cursor, pretty_error(cursor, chars));
            }
        }

        throw ers::make_parse_error("JSON array node reached the end of buffer while parsing object contents. {}",
            pretty_error(cursor, chars));
    }

    std::size_t parser::parse_escaped_unicode_codepoint(std::size_t cursor, std::string& string_value) const {
        // Note 1:
        ...
        // have to do it in order to be standard-compliant.

        // Note 2:
        // 1st surrogate contains high bits, 2nd surrogate contains low bits.

        const auto make_parsing_error = [&](std::string_view hex) {
            return ers::make_parse_error("JSON string node could not parse unicode codepoint '{}' while parsing an escape sequence at pos {}. {}",
        ...
        const auto make_end_of_buffer_error = [&]() {
            return ers::make_parse_error("JSON string node reached the end of buffer while "
        ...
        const auto parse_utf16 = [&](std::string_view hex) -> std::uint16_t {
            ...
            if (!sequence_is_valid || !sequence_parsed_fully)
                throw make_parsing_error(hex);
        ...
        const auto start = chars.data() + cursor;

        if (cursor + hex_1_end >= chars.size())
        ...
        if (0xD800 <= utf16_1 && utf16_1 <= 0xDFFF) {
            if (cursor + hex_2_end >= chars.size())
                throw make_end_of_buffer_error_for_pair();
            if (start[hex_2_backslash] != '\\')
                throw make_surrogate_error(hex_1);
            if (start[hex_2_prefix] != 'u')
                throw make_surrogate_error(hex_1);
    ...
    std::pair<std::size_t, String> parser::parse_string(std::size_t cursor) const {
        ...
        for (std::size_t segment_start = cursor; cursor < chars.size(); ++cursor) {
            ...
            if (c == '"') {
                string_value.append(chars.data() + segment_start, cursor - segment_start);
            ...
            if (c == '\\') {
                ...
                string_value.append(chars.data() + segment_start, cursor - segment_start - 1);

                // can't buffer more than that since we have to insert special characters now

                if (cursor >= chars.size()) {
                    throw ers::make_parse_error("JSON string node reached the end of buffer while parsing an escape sequence at pos {}. {}",
                        cursor, pretty_error(cursor, chars));
                }

                const char escaped_char = chars[cursor];
                ...
                // 6/12-character escape sequences (escaped unicode HEX codepoints)
                    ...
                    throw ers::make_parse_error("JSON string node encountered unexpected character '{}' while parsing an escape sequence at pos {}. {}",
            ...
            if (to_u8(c) <= 31) {
                throw ers::make_parse_error("JSON string node encountered unescaped ASCII control character character \\{} at pos {}. {}",
                    c, cursor, pretty_error(cursor, chars));
            }
        }

        throw ers::make_parse_error("JSON string node reached the end of buffer while parsing string contents. {}",
            pretty_error(cursor, chars));
    }
    ...
    std::pair<std::size_t, std::variant<Integral, Floating>> parser::parse_number(std::size_t cursor) const {
        ...
        if (result.ec != std::errc {}) {
            ...
            if (result.ec == std::errc::invalid_argument) {
                throw ers::make_parse_error("JSON floating node could not be parsed as a number at pos {}. {}",
                    cursor, pretty_error(cursor, chars));
            }

            if (result.ec == std::errc::result_out_of_range) {
                throw ers::make_parse_error("JSON number node parsed to floating larger than its possible binary representation at pos {}. {}",
                    cursor, pretty_error(cursor, chars));
            }
        }

        return { result.ptr - chars.data(), number_value };
    }

    std::pair<std::size_t, Bool> parser::parse_true(std::size_t cursor) const {
        using namespace std::string_literals;
        constexpr std::size_t token_length = 4;

        if (cursor + token_length > chars.size()) {
            throw ers::make_parse_error("JSON bool node reached the end of buffer while parsing 'true'. {}",
                pretty_error(cursor, chars));
        }

        bool parsed_correctly = chars[cursor + 0] == 't'
            && chars[cursor + 1] == 'r'
            && chars[cursor + 2] == 'u'
            && chars[cursor + 3] == 'e';

        if (!parsed_correctly) {
            throw ers::make_parse_error("JSON bool node could not parse 'true' at pos {}. {}",
                cursor, pretty_error(cursor, chars));
        }

        return { cursor + token_length, true };
    }

    std::pair<std::size_t, Bool> parser::parse_false(std::size_t cursor) const {
        using namespace std::string_literals;
        constexpr std::size_t token_length = 5;

        if (cursor + token_length > chars.size()) {
            throw ers::make_parse_error("JSON bool node reached the end of buffer while parsing 'false'. {}",
                pretty_error(cursor, chars));
        }

        const bool parsed_correctly = chars[cursor + 0] == 'f'
            && chars[cursor + 1] == 'a'
            && chars[cursor + 2] == 'l'
            && chars[cursor + 3] == 's'
            && chars[cursor + 4] == 'e';
    ...
    std::pair<std::size_t, Null> parser::parse_null(std::size_t cursor) const {
        using namespace std::string_literals;
        constexpr std::size_t token_length = 4;

        if (cursor + token_length > chars.size()) {
            throw ers::make_parse_error("JSON null node reached the end of buffer while parsing 'null'. {}",
                pretty_error(cursor, chars));
        }

        const bool parsed_correctly = chars[cursor + 0] == 'n'
            && chars[cursor + 1] == 'u'
            && chars[cursor + 2] == 'l'
            && chars[cursor + 3] == 'l';
...
namespace utl {
    Node from_string(const std::string& chars, std::size_t recursion_limit) {
        ...
        const std::size_t json_start = parser.skip_nonsignificant_whitespace(0);

        // starts parsing recursively from the root node

        auto [end_cursor, node] = parser.parse_node(json_start);
    ...
    Node literals::operator ""_json(const char* cstr, std::size_t size) {
        return from_string(std::string(cstr, size));
    }
}
