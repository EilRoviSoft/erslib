#include "erslib/modkit/dependency_graph.hpp"
...
// ers
#include <erslib/core/ranges.hpp>
#include <erslib/modkit/util/stage.hpp>


std::vector<std::string> ers::modkit::impl::resolve_mods_order(const ModContainer& mods, std::string_view initial_mod) {
    // Step 0: preparing
    ...
    for (const Mod& mod : mods) {
        for (const dependency_t& dep : mod.metadata().dependencies) {
            ...
            if (dep.type == EDependencyType::Required) {
                if (!dependency_exist) {
                ...
                add_to_graph = true;
            } else if (dep.type == EDependencyType::Optional) {
                if (dependency_exist)
                    continue;

                add_to_graph = true;
            } else if (dep.type == EDependencyType::Incompatible) {
                if (dependency_exist) {
    ...
    if (visited_count != indegree.size()) {
        throw make_dependency_error("Occured dependency cycle detected between mods: {}",
            indegree
            | views::filter([](const auto& it) { return it.second != 0; })
            | std::views::keys);
    }
...
std::vector<ers::modkit::impl::stage_order_info_t> ers::modkit::impl::resolve_stages_order(
