...
conduit::impl::ConflictUpdateClause::ConflictUpdateClause(std::vector<std::string> columns) :
    IClause(&slots::conflict_action),
    _columns(std::move(columns)) {
}

ers::Status conduit::impl::ConflictUpdateClause::render(Context& ctx) const {
    if (_columns.empty()) {
...
conduit::impl::Clause conduit::impl::clauses::on_conflict_constraint(std::string name) {
    return identifier(&slots::conflict_constraint, std::move(name));
}

conduit::impl::Clause conduit::impl::clauses::do_nothing() {
