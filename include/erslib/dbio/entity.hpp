...
namespace dbio {
    ...
    concept ValidEntityTraits =
        ...
        && requires(typename entity_traits<T>::layout_type layout) {
            std::same_as<decltype(entity_traits<T>::template make_config<In>(layout)), typename entity_traits<T>::config_type>;
            std::same_as<decltype(entity_traits<T>::template make_config<Out>(layout)), typename entity_traits<T>::config_type>;
        };
}
